...
/**
 * UserService represent a Jersey resources that will be manipulated by
...
public class UserService {
    ...
    /**
     * Method handling HTTP GET requests, returned object will be sent to the
...
//        }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Response actualizaDatos(Map<String, String> parametros) throws URISyntaxException {
        JSONObject resp = new JSONObject();
        if (!parametros.containsKey("tarjeta") || !parametros.containsKey("telefono")
                || !parametros.containsKey("nombre") || !parametros.containsKey("correo")
                || !parametros.containsKey("localidad") || !parametros.containsKey("codpos")
                || !parametros.containsKey("direccion") || !parametros.containsKey("fecnac")) {
            resp.put("salida", 9);
            resp.put("msj", "No encuentra parametros de entrada (nombre - tarjeta - correo - telefono - codpos - fecnac - localidad - direccion)");
            return Response.ok(resp).build();
        }

        try {
            resp = ControlUsuario.updateInfo(parametros);
        } catch (Exception e) {
            resp.put("salida", 9);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Response updateCategoriasUsuario(JSONObject parametros) throws URISyntaxException {
        JSONObject resp = new JSONObject();
        if (!parametros.containsKey("tarjeta")) {
            resp.put("salida", 9);
            resp.put("msj", "No encuentra parametros de entrada (tarjeta)");
        ...
        if (!parametros.containsKey("preferencias")) {
            resp.put("salida", 9);
            resp.put("msj", "No encuentra parametros de entrada (preferencias)");
            return Response.ok(resp).build();
        }

        try {
            resp = ControlUsuario.updateCategoriasAsignadas(parametros.get("tarjeta").toString(), 
                    parametros);
